...
def kmeans(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ...
        x,y,z=np.zeros(k,dtype=int),np.zeros(k),np.zeros((k,D))
        
        for i in range(N):
            for j in range(k):
                y[j]=np.linalg.norm(features[i]-centers[j])
                
            b=min(range(len(y)), key=lambda a: y[a])
            z[b],x[b],assignments[i]=z[b]+features[i],x[b]+1,b
