...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    for row in range(0, H+1, stepSize):
        for col in range(0, W+1, stepSize):
            ...
            score = np.dot(hog_feature, base_score)
    ...
    # Adjust the maximum location to be the center of the window
    maxr -= winH//2
    maxc -= winW//2

    # Resize the response map to the original image size
...
def pyramid(image, scale=0.9, minSize=(200, 100)):
    ...
    #####################################
    min_h, min_w = minSize
    while image.shape[0] > min_h and image.shape[1] > min_w:
