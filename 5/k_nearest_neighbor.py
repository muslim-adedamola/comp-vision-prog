...
def predict_labels(dists, y_train, k=1):
    ...
    #       START YOUR CODE HERE        #
    #####################################   
    closest_y = y_train[np.argsort(dists, axis=1)[:, :k]]
    y_pred = np.argmax(np.apply_along_axis(lambda x: np.bincount(x, minlength=np.max(y_train)+1), axis=1, arr=closest_y), axis=1)
    ######################################
    #        END OF YOUR CODE            #
    ######################################
...
def split_folds(X_train, y_train, num_folds):
    ...
    for i in range(num_folds):
        X_vals[i] = X_train_shuffled[i*validation_size:(i+1)*validation_size]
        y_vals[i] = y_train_shuffled[i*validation_size:(i+1)*validation_size]
        X_trains[i] = np.vstack([X_train_shuffled[:i*validation_size], X_train_shuffled[(i+1)*validation_size:]])
        y_trains[i] = np.hstack([y_train_shuffled[:i*validation_size], y_train_shuffled[(i+1)*validation_size:]])
    ######################################
    #        END OF YOUR CODE            #
    ######################################
